...
export function moveRobot(
  ...
  function flee(n) {
    ...
    if (danger(n)) {
      if (!something(util.spriteUp(n, ni))) {
        d = "up";
      } else if (!something(util.spriteDown(n, ni))) {
        d = "down";
    ...
    if (danger(util.spriteUp(n, ni)) || danger(util.spriteDown(n, ni))) {
  ...
  function something(n) {
    const i = util.getI(n, ni);
    const j = util.getJ(n, ni);
    if (decor[n].image !== "") {
      return true;
    }
    if (
  ...
  function nDirection(d, n) {
    ...
    switch (d) {
      case "up":
        n2 = util.spriteUp(n, ni);
        break;
      case "down":
        n2 = util.spriteDown(n, ni);
        break;
      case "left":
  ...
  const nRobot = util.getIndex(iRobot, jRobot, ni);
  let d1 = "";
  let d2 = "";

  if (danger(nRobot)) {
    setDisplacement(flee(nRobot));
    d1 = "danger";
  } else {
    d1 = flee(nRobot);
